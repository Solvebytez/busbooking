...
const useFilterData = (filterData: FilterData) => {
  ...
  console.log("FilterData From Query",filterData);

  // Define the query key and query function
  const queryKey = ['filterData', filterData];
  
  const queryFn = async () => {
    try {
      const response = await axiosInstance.post('bus/schedules/list/', filterData);
  ...
  return useQuery<FilterResponse, Error>({queryKey, queryFn});
};
